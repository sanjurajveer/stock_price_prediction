...
def preprocess_data(df):
    df['SMA_50'] = df['Close'].rolling(window=50).mean() #50 days simple moving avrg, it smooths price fluctuation to detect the trends
    df['EMA_50'] = df['Close'].ewm(span=20, adjust=False).mean() #20 dyas exp moving avg, more weight to recent prices, making it responsive to market changes
    df['Daily_Return'] = df['Close'].pct_change() #shortt tem trend
    df['Log_Return'] = np.log(df['Close'] / df['Close'].shift(1)) #logrithmic returns help to mode finanical as it handles compounding
    
    delta = df['Close'].diff(1) #rsi
    gain = (delta.where(delta >0,0)).rolling(window=14).mean()
    loss = (-delta.where(delta<0,0)).rolling(window=14).mean()
    rs = gain / loss
    df['RSI_14'] = 100 -(100 / (1 + rs)) #momentum in market
    ...
    df['Momentum'] = df['Close'] - df['Close'].shift(4) #value above 70=overbought and <30 is oversold
...
def normalize_data(df):
    scaler = MinMaxScaler() #scales the mean price
    scaled_data = scaler.fit_transform(df[['Close']])
    return scaled_data, scaler

def prepare_data(scaled_data, time_steps=60): #time series prediction
    X, y = [], []
...
def build_model(input_shape): #nueral network stacked one above another
    model = Sequential([
